...
const TrackStatus: React.FC = () => {
      ...
      return (
        <div className="min-h-screen bg-[#F8F6F1] px-4 py-12 flex items-center justify-center">
          <div className="max-w-2xl mx-auto bg-red-50 border border-red-200 rounded-2xl p-6 text-center">
            ...
            <Link to="/track" className="mt-4 inline-block text-[#D97706] hover:text-[#b85f00] font-bold text-sm transition-colors">
              ← Try another ID
            </Link>
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen bg-[#F8F6F1]">
        <div className="max-w-4xl mx-auto px-4 py-8">
          <Link
            to="/track"
            className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-[#1A365D] transition-colors font-medium mb-6 group"
          >
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden">
            {/* Header */}
            <div className="bg-gradient-to-r from-[#1A365D] to-[#2D4A6B] px-6 py-5 text-white">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-2xl">construction</span>
                    <h1 className="text-xl font-display font-bold tracking-tight">Repair Status</h1>
                  </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-slate-50/80 rounded-xl p-4 border border-slate-200/60">
                  ...
                  <p className="text-base font-semibold text-slate-800">{repairData.customerName}</p>
                  ...
                  {steps.map((step, idx) => (
                    <div key={idx} className="relative flex items-start gap-4 pb-6 last:pb-0">
                      ...
                      <div className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${step.status === 'completed' ? 'bg-emerald-500 text-white' :
                          step.status === 'current' ? 'bg-[#1A365D] text-white shadow-md shadow-[#1A365D]/30' :
                      ...
                      <div className="flex-1 pt-0.5">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {step.status === 'current' && (
                            <span className="text-[10px] font-bold text-[#1A365D] bg-[#1A365D]/10 px-2 py-0.5 rounded-full border border-[#1A365D]/20">
                              In Progress
                            </span>
                          )}
  ...
  // Default view: search form
  return (
    <div className="min-h-screen bg-[#F8F6F1] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-6">
          <div className="text-center mb-6">
            <div className="w-16 h-16 rounded-2xl bg-[#1A365D] flex items-center justify-center mx-auto shadow-lg shadow-[#1A365D]/20">
              <span className="material-symbols-outlined text-3xl text-white">search</span>
            </div>
            <h1 className="text-2xl font-display font-bold text-[#1A365D] mt-3">Track Your Repair</h1>
          ...
          <form onSubmit={handleTrack}>
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-12 pl-10 pr-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-[#1A365D] focus:bg-white outline-none transition-all text-sm text-slate-800 placeholder-slate-400 font-medium"
                required
              />
            </div>
            <button
              type="submit"
              disabled={isLoading}
              className="w-full mt-4 h-12 bg-[#1A365D] text-white font-bold rounded-xl hover:bg-[#2D4A6B] active:scale-[0.98] transition-all flex items-center justify-center gap-2 text-sm shadow-sm shadow-[#1A365D]/20 disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <>
          ...
          <div className="mt-6 text-center">
            <p className="text-[10px] text-slate-400">Your ticket ID is provided on your repair receipt or confirmation email.</p>
            <p className="text-[10px] text-slate-400 mt-1">For privacy, customer details are partially masked.</p>
          </div>
        </div>
      </div>
    </div>
